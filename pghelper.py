...
class PGHelper:
    def __init__(self,dbname = '',user = '',password = '',host = '',port = 5432):
        ...
        if os.path.exists(config.latency_buffer_path):
        ...
        elif os.path.exists(config.total_latency_buffer):
            tmp_buffer_file = open(config.total_latency_buffer,"r")
            lines = tmp_buffer_file.readlines()
            for line in lines:
                data = json.loads(line)
                if data[0] not in self.latencyTotalBuffer:
                    self.latencyTotalBuffer[data[0]] = {}
                self.latencyTotalBuffer[data[0]][data[1]] = data[2]
            self.buffer_file = open(config.latency_buffer_path,"a")
        elif os.path.exists(config.pg_latency):
