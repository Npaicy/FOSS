...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(log_dir=config.runstate)
    ...
    querymanager = QueryManager(writer = writer,planhelper=planhelper)
    ...
    # RUN NOW
    if not os.path.exists(config.pg_latency):
        shutil.copy(config.latency_buffer_path,config.pg_latency)
    #  =========Train config===========
    ...
    #===================Explore=====================
        ...
        #=================Validate Phase================
                ...
                for query_id,hints in candidatebest.items():
                    if hints:
                        ...
                        for hint,steps in hints:
                            ...
                            print('Query id:{}, CurrBest:{:.3f}, EstiBest:{:.3f}, Advbybest:{:.3f}%'.format(query_id,bestexec,latency_timeout[0],Advbybest * 100))
            ...
            freq = 5
            if iter >= config.startiter and test_iter % freq == 0:
                while True:
                    sql, queryno, query_id, oneloop = querymanager.get2validate()
