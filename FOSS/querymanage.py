...
# @ray.remote
class QueryManager:
    def __init__(self,writer = None,planhelper = None) -> None:
        ...
        for i in range(len(self.test_dir)):
            _,_,left_deep,_ = self.planhelper.get_feature('',sql,True,query_id=query_id)
            if not left_deep:
                print(f'Query:{query_id} is not left-deep ')
        ...
        if not os.path.exists(config.encoding_path):
